...
const validationError = function (source, options, configFile, policyFiles) {
  if (source === os.tmpdir()) {
...
class OptionsValidator {
  ...
  validationError(source, options, configFile, policyFiles) {
    if (source === os.tmpdir()) {
      return 'Source directory is the Node temp directory. Cowardly refusing to fill up disk with recursive copy.';
    }
    if (!options.region) {
  ...
  checkForValidationErrors(source, options, configFile, policyFiles) {
    this.errorMessage = this.validationError(source, options, configFile, policyFiles);
  };
